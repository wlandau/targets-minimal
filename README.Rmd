...
For every GitHub push on main branch, the `targets` pipeline runs on [GitHub Actions](https://github.com/wlandau/targets-minimal/actions/workflows/run.yaml). All you need is to define a `.github/workflows/run.yaml` file that caches the `_targets/` data store and runs the pipeline.
...
Each subsequent run restores the previous cache and skips targets that are already up to date.

![](./man/figures/uptodate.png)

Read [this explanation](https://docs.github.com/en/actions/guides/caching-dependencies-to-speed-up-workflows#usage-limits-and-eviction-policy) for usage restrictions and limitations of GitHub Actions caches.
