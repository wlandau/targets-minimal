...
The [`targets`](https://github.com/wlandau/targets) R package manages the workflow. It automatically skips steps of the pipeline when the results are already up to date, which is critical for larger projects. [`targets`](https://github.com/wlandau/targets) also helps users understand their workflows with interactive dependency graphs (screenshot below).
...
1. Run the `targets` pipeline by typing [`tar_make()`](https://wlandau.github.io/targets/reference/tar_make.html) in the R console.
1. View the results with [`tar_load()`](https://wlandau.github.io/targets/reference/tar_load.html) or [`tar_read()`](https://wlandau.github.io/targets/reference/tar_read.html), e.g. `tar_read(hist)`.
