...
This pipeline runs in a [GitHub Actions workflow](https://github.com/wlandau/targets-minimal/actions) on every push to the main branch. The [cache action](https://github.com/actions/cache) restores the latest `_targets/` data store and rendered `index.html` file before each run so that up-to-date targets do not rebuild. The latest run with `_targets/` and `index.html` is overwritten in the [`latest-run`](https://github.com/wlandau/targets-minimal/tree/latest-run) branch, and the R Markdown report is deployed to <https://wlandau.github.io/targets-minimal/> using [GitHub Pages](https://pages.github.com/). To activate similar continuous deployment in your own projects, simply copy [`.github/workflows/targets.yaml`](https://github.com/wlandau/targets-minimal/blob/main/.github/workflows/targets.yaml) to your own repo. Please read about the [storage limitations described here](https://docs.github.com/en/actions/guides/caching-dependencies-to-speed-up-workflows#usage-limits-and-eviction-policy).
